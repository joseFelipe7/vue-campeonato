  ...
  <div class="container my-3">
    <div class="row">
        <div class="col-lg-4 my-2" v-for="(item, index) in championships" :key="item.id">   
            <div class="card-championship-info" >
               <div class="mb-3">
                   <h3 class="text-white text-center">{{item.name}}</h3> 
               </div>
               <div>
                    <p class="text-white">Player vencedor: {{item.player_win?item.player_win:'Campeonato em andamento'}}</p>
                    <p class="text-white">Quantidade de players: {{item.total_players}}</p>
                    <p class="text-white">Rodada atual: {{item.round_current?item.round_current:'Não iniciado'}}</p>
                    <p class="text-white">Total de rodadas: {{item.round_total}}</p>
               </div>
...
<style scoped>
.card-championship-info.card-championship-info{
  padding: 20px 40px; 
  background: #1A0831; 
  display: flex; 
